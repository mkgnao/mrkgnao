<?php namespace App\Http\TeamWorkPm\People;

class Status extends \App\Http\TeamWorkPm\Rest\Model
{
    ...
    public function insert(array $data)
    {
        ...
        if ($person_id <= 0) {
            throw new \App\Http\TeamWorkPm\Exception('Required field person_id');
        }
    ...
    public function update(array $data)
    {
        ...
        if ($id <= 0) {
            throw new \App\Http\TeamWorkPm\Exception('Required field id');
        }
        $person_id = empty($data['person_id']) ? 0 : (int) $data['person_id'];
    ...
    public function delete($id, $person_id = null)
    {
        $id = (int) $id;
        if ($id <= 0) {
            throw new  \App\Http\TeamWorkPm\Exception('Invalid param id');
        }
